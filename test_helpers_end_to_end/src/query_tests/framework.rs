//! The common test code that drives the tests in the `influxdb_iox/tests/query_tests/` module

use crate::{maybe_skip_integration, MiniCluster, Step, StepTest};
...
impl TestCase {
    ...
    pub async fn run_and_print_runtime(&self, print_runtime: bool) {
        ...
        for chunk_stage in self.chunk_stage {
            ...
            let input_path: PathBuf = self.input.into();
            ...
            let test_step = match input_path.extension() {
            ...
            .run_and_print_runtime(print_runtime)
            .await;
        }
    }
}
