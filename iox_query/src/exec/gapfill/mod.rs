...
impl UserDefinedLogicalNodeCore for GapFill {
    ...
    fn necessary_children_exprs(&self, _output_columns: &[usize]) -> Option<Vec<Vec<usize>>> {
        // we may not need to compute the indices here.
        // returning Some(vec![vec![]]) might be sufficient,
        // since the child Aggregate plan should reference the same columns as well
        let indices = self
            .expressions()
            .iter()
            .filter_map(|expr| {
                Some(
                    expr.to_columns()
                        .ok()?
                        .into_iter()
                        .filter_map(|col| self.input.schema().maybe_index_of_column(&col)),
                )
            })
            .flatten()
            .collect::<Vec<_>>();
