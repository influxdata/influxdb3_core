...
pub struct ObjectStoreCacheMetrics {
    ...
    /// Track seen items.
    once_contained: Mutex<Bloom2<RandomState, CompressedBitmap, String>>,
...
impl ObjectStoreCacheMetrics {
    /// Create a new [`ObjectStoreCacheMetrics`] sized appropriately for a
    /// max_number_of_unique_objects and desired `false_positive_rate` (as a nonzero percentage).
    pub fn new(
        inner: Arc<dyn ObjectStore>,
        registry: &Registry,
        store_type: StoreType,
        max_number_of_unique_objects: NonZeroUsize,
        false_positive_rate: NonZeroUsize,
    ) -> Self {
        ...
        let bytes_novel =
            bytes.recorder([("store", store_type.0.clone()), ("state", "novel".into())]);
        ...
        Self {
            ...
            once_contained: lock_metrics
                .new_mutex(BloomFilterBuilder::default().size(filter_size).build()),
            ...
            .and_then(|val| CacheState::try_from(val).ok())
        {
            match state {
                CacheState::NewEntry => {
                    ...
                    if self.maybe_seen(&location) {
                        self.bytes_thrashed.record(size as u64)
                    } else {
                        self.bytes_novel.record(size as u64)
                    }
                }
                CacheState::AlreadyLoading | CacheState::WasCached => {
    ...
    /// Determine if the object was previously cached.
    /// Returns a true if key may have been seen previously.
    ///
    /// The false positive rate (or likelihood of falsing determining a key was previously seen)
    /// is based upon the anticipated `max_number_of_unique_objects`
    /// provided to the [`ObjectStoreCacheMetrics::new`].
    /// Refer to [`set_bloom_filter_sizing`] for the calculation.
    fn maybe_seen(&self, location: &String) -> bool {
        let mut guard = self.once_contained.lock();
    ...
    pub(crate) fn replace_inner_store(&self, inner: Arc<dyn ObjectStore>) -> Arc<Self> {
        let guard = self.once_contained.lock();
        Arc::new(Self {
            inner,
            once_contained: Mutex::new(guard.clone()),
            bytes_thrashed: self.bytes_thrashed.clone(),
            bytes_cache_hit: self.bytes_cache_hit.clone(),
            bytes_novel: self.bytes_novel.clone(),
        })
    }
}
...
mod tests {
    ...
    async fn test_cache_metrics() -> Result<()> {
        ...
        let num_objects = NonZeroUsize::new(100).unwrap();
        ...
        assert_u64histogram_hits(
            &metrics,
            CACHE_METRIC_BYTES,
            [("store", "without_cache"), ("state", "novel")],
            0,
        );
    ...
    async fn test_cache_metrics_can_record_thrash() -> Result<()> {
        ...
        assert_u64histogram_hits(
            &metrics,
            CACHE_METRIC_BYTES,
            [("store", "owned_cache_metrics_store"), ("state", "novel")],
            3,
        );
