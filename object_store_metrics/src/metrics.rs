...
{
    #[must_use]
    pub(crate) fn new(metrics: Arc<M>, time_provider: &Arc<dyn TimeProvider>) -> Self {
        Self {
            metrics,
    ...
    M: AsRef<Metrics> + AsRef<MetricsWithBytes> + std::fmt::Debug + Send + Sync,
{
    ...
    pub(crate) fn new(metrics: Arc<M>, time_provider: &Arc<dyn TimeProvider>) -> Self {
        Self {
            inner: MetricsRecorder::new(metrics, time_provider),
            bytes: None,
        }
    }
        ...
        + Sync,
{
    ...
    pub(crate) fn new(metrics: Arc<M>, time_provider: &Arc<dyn TimeProvider>) -> Self {
        Self {
            inner: MetricsWithBytesRecorder::new(metrics, time_provider),
            t_first_byte: None,
        }
    }
        ...
        + Sync,
{
    fn drop(&mut self) {
            ...
            .and_then(|t_first_byte| t_first_byte.checked_duration_since(self.inner.inner.t_begin))
        {
            ...
            metrics.duration.get(self.inner.inner.op_res).record(delta);
        }
    }
}
