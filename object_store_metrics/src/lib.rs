...
mod cache_metrics;
pub use cache_metrics::ObjectStoreCacheMetrics;
pub mod cache_state;
mod metrics;
...
impl ObjectStore for ObjectStoreMetrics {
    async fn put(&self, location: &Path, payload: PutPayload) -> Result<PutResult> {
        let mut recorder =
            MetricsWithBytesRecorder::new(Arc::clone(&self.put), &self.time_provider);
    ...
    async fn put_opts(
        ...
        let mut recorder =
            MetricsWithBytesRecorder::new(Arc::clone(&self.put_opts), &self.time_provider);
    ...
    async fn put_multipart(&self, location: &Path) -> Result<Box<dyn MultipartUpload>> {
        let recorder =
            MetricsWithBytesRecorder::new(Arc::clone(&self.put_multipart), &self.time_provider);
    ...
    async fn get(&self, location: &Path) -> Result<GetResult> {
        let recorder =
            MetricsWithBytesAndTtfbRecorder::new(Arc::clone(&self.get), &self.time_provider);
    ...
    async fn get_opts(&self, location: &Path, options: GetOptions) -> Result<GetResult> {
        let recorder =
            MetricsWithBytesAndTtfbRecorder::new(Arc::clone(&self.get_opts), &self.time_provider);
    ...
    async fn get_range(&self, location: &Path, range: Range<usize>) -> Result<Bytes> {
        let mut recorder =
            MetricsWithBytesRecorder::new(Arc::clone(&self.get_range), &self.time_provider);
    ...
    async fn get_ranges(&self, location: &Path, ranges: &[Range<usize>]) -> Result<Vec<Bytes>> {
        let mut recorder =
            MetricsWithBytesRecorder::new(Arc::clone(&self.get_ranges), &self.time_provider);
    ...
    async fn head(&self, location: &Path) -> Result<ObjectMeta> {
        let mut recorder = MetricsRecorder::new(Arc::clone(&self.head), &self.time_provider);
    ...
    async fn delete(&self, location: &Path) -> Result<()> {
        let mut recorder = MetricsRecorder::new(Arc::clone(&self.delete), &self.time_provider);
    ...
    fn delete_stream<'a>(
        ...
        let recorder =
            MetricsWithCountRecorder::new(Arc::clone(&self.delete_stream), &self.time_provider);
    ...
    fn list(&self, prefix: Option<&Path>) -> BoxStream<'_, Result<ObjectMeta>> {
        let recorder = MetricsWithCountRecorder::new(Arc::clone(&self.list), &self.time_provider);
    ...
    fn list_with_offset(
        ...
        let recorder =
            MetricsWithCountRecorder::new(Arc::clone(&self.list_with_offset), &self.time_provider);
    ...
    async fn copy(&self, from: &Path, to: &Path) -> Result<()> {
        let mut recorder = MetricsRecorder::new(Arc::clone(&self.copy), &self.time_provider);
    ...
    async fn rename(&self, from: &Path, to: &Path) -> Result<()> {
        let mut recorder = MetricsRecorder::new(Arc::clone(&self.rename), &self.time_provider);
    ...
    async fn copy_if_not_exists(&self, from: &Path, to: &Path) -> Result<()> {
        let mut recorder =
            MetricsRecorder::new(Arc::clone(&self.copy_if_not_exists), &self.time_provider);
    ...
    async fn rename_if_not_exists(&self, from: &Path, to: &Path) -> Result<()> {
        let mut recorder =
            MetricsRecorder::new(Arc::clone(&self.rename_if_not_exists), &self.time_provider);
...
async fn wrap_getresult_res(
    ...
    match res {
        Ok(mut res) => {
            res.payload = match res.payload {
                GetResultPayload::File(file, path) => {
                    ...
                    let first_bytes = recorder.freeze();
                    recorder.submit(Some(first_bytes), OpResult::Success, size);
                    GetResultPayload::File(file, path)
                }
                GetResultPayload::Stream(s) => {
                    ...
                    GetResultPayload::Stream(Box::pin(Box::new(
                        StreamMetricRecorder::new(s, BytesStreamDelegate::new(recorder)).fuse(),
                    )))
                }
            };
            Ok(res)
        ...
        Err(e) => {
            recorder.submit(None, OpResult::Error, None);
            Err(e)
        }
    }
}
