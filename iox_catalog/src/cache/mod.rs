...
impl NamespaceRepo for Repos {
    ...
    async fn get_storage_by_id(
        ...
        match self.namespaces.get(id).await {
            Ok(n) => {
                let ns = n.namespace()?;
                if ns.deleted_at.is_some() && deleted == SoftDeletedRows::ExcludeDeleted {
                    return Ok(None);
                }
    ...
    async fn get_storage_by_id(
        ...
        match self.tables.get(table_id).await {
            Ok(table_snapshot) => {
                ...
                if table.deleted_at.is_some() && deleted == SoftDeletedRows::ExcludeDeleted {
                ...
                let column_count = self.columns().list_by_table_id(table_id).await?.len() as i64;
