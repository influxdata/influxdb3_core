...
use crate::{
    interface::{
        CatalogStorage, Error, NamespaceSorting, Paginated, PaginationOptions, Result, TableSorting,
    },
...
impl GrpcCatalogStorageClientBuilder {
    ...
    pub fn build(self) -> GrpcCatalogStorageClient {
        ...
        let channel = TraceService::new_client(channel, req_metrics, None, "catalog_storage");
...
impl CatalogStorage for GrpcCatalogStorageClient {
    async fn get_namespace_with_storage(
        ...
        let req = proto::GetNamespaceWithStorageRequest { id: id.get() };
    ...
    async fn get_table_with_storage(&self, id: TableId) -> Result<Option<TableWithStorage>> {
        let req = proto::GetTableWithStorageRequest { table_id: id.get() };
