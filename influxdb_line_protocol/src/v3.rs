...
mod tests {
    use crate::{
        v3::{parse_lines, ParsedLine, SeriesValue},
        Error, EscapedStr, FieldValue,
    };
    ...
    fn parse_line_with_series_key() {
        let input = "cpu region/us-east/host/a1 usage=42.0,temp=10 1234";
        let vals = parse(input).unwrap();
        assert_eq!(vals[0].series.measurement, "cpu");
        let sk = vals[0].series.series_key.as_ref().unwrap();
        assert_eq!(sk[0].0, "region");
        assert_eq!(sk[0].1, "us-east");
        assert_eq!(sk[1].0, "host");
        assert_eq!(sk[1].1, "a1");
        let fs = &vals[0].field_set;
        assert_eq!(fs.len(), 2);
        assert_eq!(fs[0].0, "usage");
        assert!(matches!(fs[0].1, FieldValue::F64(42.0)));
        let ts = &vals[0].timestamp;
        assert!(matches!(ts, Some(1234)));
    }
