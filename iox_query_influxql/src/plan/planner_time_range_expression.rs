...
fn lower_bound_to_df_expr(tz: &Option<Arc<str>>, v: Option<i64>) -> Option<DFExpr> {
    v.map(|ts| {
        "time"
            .as_expr()
...
/// Returns a datafusion expression in UTC, relying upon the datafusion
/// engine to perform any casting (to other timezones) as needed.
fn upper_bound_to_df_expr(tz: &Option<Arc<str>>, v: Option<i64>) -> Option<DFExpr> {
    v.map(|ts| {
        "time"
            .as_expr()
...
/// Returns a datafusion expression in UTC, relying upon the datafusion
/// engine to perform any casting (to other timezones) as needed.
pub(super) fn time_range_to_df_expr(tz: &Option<Arc<str>>, cond: TimeRange) -> Option<DFExpr> {
    match (cond.lower, cond.upper) {
        (Some(lower), Some(upper)) if lower == upper => {
            // Use ts = <lower> if the lower and upper are equal
            Some("time".as_expr().eq(lit(ScalarValue::TimestampNanosecond(
                Some(lower),
                tz.clone(),
            ))))
        }
        ...
        (lower, upper) => match (
            lower_bound_to_df_expr(tz, lower),
            upper_bound_to_df_expr(tz, upper),
        ) {
