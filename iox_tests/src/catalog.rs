...
impl TestPartition {
    ...
    pub async fn create_parquet_file_in_object_storage(
        ...
        assert!(
            column_set.is_none(),
            "Cannot have both a record batch and a manually set column_set!"
        );
        ...
        TestParquetFileBuilder {
            ...
            column_set: None, // will be computed from the record batch again
            max_l0_created_at,
            partition_hash_id_override_to_none,
        }
    }
    ...
    pub async fn create_parquet_file_catalog_record(
        ...
        let (row_count, column_set) = if let Some(record_batch) = record_batch {
            ...
            let column_set =
                column_set.unwrap_or_else(|| ColumnSet::new(table_catalog_columns.ids()));
...
pub struct TestParquetFileBuilder {
    ...
    pub column_set: Option<ColumnSet>,
